...
from talos.exceptions import EnvironmentNotSupportedException, AgentAlreadyExistsException, NotInCorrectCategoryChannelException
...
class Env(commands.Cog):
    ...
    async def env(self, ctx):
        """
            Group of commands for interacting with RL environments
        """
    ...
    async def ls(self, ctx):
        """
            Lists environments supported
        """
        await ctx.channel.send("Προτιμώ τα Atari αλλά τέλος πάντων... {}".format(" ".join(list(get_supported_envs()))))
    
    @env.command()
    async def describe(self, ctx, env_name):
        """
            Describes the specified environment
        """
    ...
    @describe.error
    async def describe_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.send_help(ctx.command)

        if isinstance(error.original, EnvironmentNotSupportedException):
            await ctx.channel.send("Ένεκαμα τρέιν τζαμέ κόμα.  Try another environemnt or use !help!")

    @env.command()
    async def train(self, ctx, env_name):
        """
    ...
    async def train_error(self, ctx, error):
        ...
        elif isinstance(error.original, AgentAlreadyExistsException):
